...
from unet3dStep2 import losses
from unet3dStep2.utils.utils import load_json, dump_json, get_class, get_kwargs
from unet3dStep2.models.build import build_or_load_model
...
def build_data_loaders(config, output_dir, dataset_class, metric_to_monitor="val_loss",
    ...
    # Create datasets
    training_labeled_dataset = dataset_class(filenames=config['training_labeled_filenames'],
                                     **training_kwargs,
                                     **dataset_kwargs)

    training_labeled_loader = DataLoader(training_labeled_dataset,
    ...
    training_unlabeled_dataset = dataset_class(filenames=config['training_unlabeled_filenames'],
                                     **training_kwargs,
                                     **dataset_kwargs)

    training_unlabeled_loader = DataLoader(training_unlabeled_dataset,
                                 batch_size=batch_size,
                                 shuffle=True,
                                 num_workers=n_workers,
                                 pin_memory=pin_memory,
    ...
    if test_input:
        write_dataset_examples(test_input, training_labeled_dataset, output_dir=os.path.join(output_dir, "data_loader_testing"))
    ...
    return training_labeled_loader, training_unlabeled_loader, validation_loader, metric_to_monitor
