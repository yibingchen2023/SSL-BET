...
from unet3dStep3.train import run_training
from unet3dStep3.utils.utils import load_json
from unet3dStep3.predict import volumetric_predictions
from unet3dStep3.utils.filenames import load_dataset_class
from uunet3dStep3.scripts.script_utils import (get_machine_config, add_machine_config_to_parser, build_optimizer,
...
def run(config_filename, output_dir, namespace):
    ...
    if "cross_validation" in config:
        ...
        training_labeled_loader, training_unlabeled_loader, validation_loader, metric_to_monitor = build_data_loaders_from_config(config,
                                                                                               system_config,
                                                                                               work_dir,
                                                                                               dataset_class)
        ...
        model_one = build_or_load_model_from_config(config,
                                                pretrained,
                                                system_config["n_gpus"])
        
        model_two = build_or_load_model_from_config(config,
                                                pretrained,
                                                system_config["n_gpus"])
        # 将两个模型的最后几层用kaiming normal初始化参数
        nn.init.kaiming_normal_(model_one.output_block.conv.conv.weight, mode='fan_in',
                                nonlinearity='leaky_relu')
        nn.init.kaiming_normal_(model_one.skip_layers.next_layer.next_layer.next_layer.next_layer.downsample.conv1.conv.weight, mode='fan_in',
                                nonlinearity='leaky_relu')
        nn.init.kaiming_normal_(model_one.skip_layers.next_layer.next_layer.next_layer.next_layer.downsample.conv2.conv.weight, mode='fan_in',
                                nonlinearity='leaky_relu')
        # nn.init.kaiming_normal_(model_one.skip_layers.next_layer.next_layer.next_layer.next_layer.next_layer.conv1.conv.weight, mode='fan_in',
        #                         nonlinearity='leaky_relu')
        # nn.init.kaiming_normal_(model_one.skip_layers.next_layer.next_layer.next_layer.next_layer.next_layer.conv2.conv.weight, mode='fan_in',
        #                         nonlinearity='leaky_relu')
        nn.init.kaiming_normal_(model_one.skip_layers.next_layer.next_layer.next_layer.next_layer.upsample.transp_conv.conv.weight, mode='fan_in',
                                nonlinearity='leaky_relu')
        nn.init.kaiming_normal_(model_one.skip_layers.next_layer.next_layer.next_layer.next_layer.upsample.conv_block.conv1.conv.weight, mode='fan_in',
                                nonlinearity='leaky_relu')
        nn.init.kaiming_normal_(model_one.skip_layers.next_layer.next_layer.next_layer.next_layer.upsample.conv_block.conv2.conv.weight, mode='fan_in',
                                nonlinearity='leaky_relu')

        nn.init.kaiming_normal_(model_two.output_block.conv.conv.weight, mode='fan_in',
                                nonlinearity='leaky_relu')
        nn.init.kaiming_normal_(model_two.skip_layers.next_layer.next_layer.next_layer.next_layer.downsample.conv1.conv.weight, mode='fan_in',
                                nonlinearity='leaky_relu')
        nn.init.kaiming_normal_(model_two.skip_layers.next_layer.next_layer.next_layer.next_layer.downsample.conv2.conv.weight, mode='fan_in',
                                nonlinearity='leaky_relu')
        # nn.init.kaiming_normal_(model_two.skip_layers.next_layer.next_layer.next_layer.next_layer.next_layer.conv1.conv.weight, mode='fan_in',
        #                         nonlinearity='leaky_relu')
        # nn.init.kaiming_normal_(model_two.skip_layers.next_layer.next_layer.next_layer.next_layer.next_layer.conv2.conv.weight, mode='fan_in',
        #                         nonlinearity='leaky_relu')
        nn.init.kaiming_normal_(model_two.skip_layers.next_layer.next_layer.next_layer.next_layer.upsample.transp_conv.conv.weight, mode='fan_in',
                                nonlinearity='leaky_relu')
        nn.init.kaiming_normal_(model_two.skip_layers.next_layer.next_layer.next_layer.next_layer.upsample.conv_block.conv1.conv.weight, mode='fan_in',
                                nonlinearity='leaky_relu')
        nn.init.kaiming_normal_(model_two.skip_layers.next_layer.next_layer.next_layer.next_layer.upsample.conv_block.conv2.conv.weight, mode='fan_in',
                                nonlinearity='leaky_relu')
        ...
        optimizer_one = build_optimizer(optimizer_name=config["optimizer_one"].pop("name"),
                                    model_parameters=model_one.parameters(),
                                    **config["optimizer_one"])
        optimizer_two = build_optimizer(optimizer_name=config["optimizer_two"].pop("name"),
                                    model_parameters=model_two.parameters(),
                                    **config["optimizer_two"])
        scheduler_one = build_scheduler_from_config(config, optimizer_one)
        scheduler_two = build_scheduler_from_config(config, optimizer_two)
        ...
        run_training(model_one=model_one.train(), 
                     model_two=model_two.train(),
                     optimizer_one=optimizer_one, 
                     optimizer_two=optimizer_two,
                     criterion=criterion,
                     n_epochs=in_config("n_epochs", config["training"], 1000),
                     training_labeled_loader=training_labeled_loader, 
                     training_unlabeled_loader=training_unlabeled_loader, 
                     ...
                     scheduler_one=scheduler_one,
                     scheduler_two=scheduler_two,
        ...
        for _dataloader, _name in build_inference_loaders_from_config(config,
            ...
            volumetric_predictions(model=model_one,
    ...
    logging.info("Total time elapsed: %.2f seconds", time.time() - start_time)  # 添加这行
