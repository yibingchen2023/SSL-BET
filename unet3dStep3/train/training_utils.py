...
def epoch_training(training_labeled_loader, training_unlabeled_loader,
    ...
    for i_labeled, item_labeled in enumerate(training_labeled_loader):
        ...
        seg_loss_one, seg_loss_two = batch_loss_train_labeled(
                                      model_one, 
                                      model_two,
                                      images_labeled,
                                      target, 
        ...
        seg_losses_one.update(seg_loss_one.item(), batch_size_labeled)
        seg_losses_two.update(seg_loss_two.item(), batch_size_labeled)
        
        # compute gradient and do optimizing step        
        optimizer_one.zero_grad()   
        optimizer_two.zero_grad()
        if scaler:
            scaler.scale(seg_loss_one).backward()
            scaler.step(optimizer_one)
            scaler.update()
            scaler.scale(seg_loss_two).backward()
            scaler.step(optimizer_two)
            scaler.update()
        else:
            # compute gradient and do step
            seg_loss_one.backward()
            optimizer_one.step()
            seg_loss_two.backward()
            optimizer_two.step()
        
        i += 1
        del seg_loss_one, seg_loss_two
    ...
    for i_unlabeled, item_unlabeled in enumerate(training_unlabeled_loader):
        ...
        pseudo_target = item_unlabeled["label"]
        images_unlabeled = trans_gau_noise(images_unlabeled, randomize=True)       
        
        cps_loss_one, cps_loss_two, \
            seg_loss_one, seg_loss_two = batch_loss_train_unlabeled(
                                      model_one, 
                                      model_two,
                                      images_unlabeled,
                                      pseudo_target,
        ...
        cps_loss_weight = 0.1
        seg_loss_weight = 0.1
        unlabeled_loss_one = cps_loss_weight * cps_loss_one + seg_loss_weight * seg_loss_one
        unlabeled_loss_two = cps_loss_weight * cps_loss_two + seg_loss_weight * seg_loss_two
        ...
        optimizer_two.zero_grad()
        if scaler:
            scaler.scale(unlabeled_loss_one).backward()
            scaler.step(optimizer_one)
            scaler.update()
            scaler.scale(unlabeled_loss_two).backward()
            scaler.step(optimizer_two)
            scaler.update()
        else:
            # compute gradient and do step
            unlabeled_loss_one.backward()
            optimizer_one.step()
            unlabeled_loss_two.backward()
            optimizer_two.step()
...
def batch_loss_train_unlabeled(model_one, model_two, 
    ...
    if n_gpus is not None:
        ...
        pseudo_target_var = torch.autograd.Variable(pseudo_target.cuda())
    ...
    if use_amp:
        ...
        with autocast():            
            ...
            return cps_loss_one, cps_loss_two, seg_loss_one, seg_loss_two
    else:        
        cps_loss_one = _batch_loss_unlabeled(
                                   model_one,
                                   model_two,
